...
COLOR_PANEL = (20, 20, 20)        # Fondo del panel
COLOR_BORDE_PANEL = (100, 100, 100)
...
pygame.display.set_caption("Aspirador Autónomo Inteligente")
pygame.font.init()
...
# ============================
aspiradoras = [
    (random.randint(0, FILAS - 1), random.randint(0, COLUMNAS - 1))
    for _ in range(NUMERO_ASPIRADORAS)
]

# ============================
# Función para mover el aspirador
# ============================
...
def mostrar_estadisticas(pantalla, movimientos, sucias, tiempo, finalizado):
    # Crear panel transparente
    panel_w, panel_h = 180, 100
    panel_x, panel_y = 10, 10

    panel = pygame.Surface((panel_w, panel_h), pygame.SRCALPHA)
    panel.fill((0, 0, 0, 150))  # Negro con transparencia
    ...
    if finalizado:
        mensaje = fuente.render("¡Limpieza completada!", True, (0, 255, 255))
...
while ejecutando:
    ...
    if not limpieza_completa:
        nuevas_posiciones = []
        for x, y in aspiradoras:
            if entorno[x][y] == 1:
                entorno[x][y] = 0
                nuevas_posiciones.append((x, y))
            else:
                nuevo_x, nuevo_y = mover_aspirador(x, y)
                nuevas_posiciones.append((nuevo_x, nuevo_y))
                movimientos_agentes += 1
        aspiradoras = nuevas_posiciones
    ...
    for i in range(FILAS):
        for j in range(COLUMNAS):
            ...
            pygame.draw.rect(
                pantalla,
                color,
                (j * TAMANIO_CELDA, i * TAMANIO_CELDA, TAMANIO_CELDA, TAMANIO_CELDA),
            )
    ...
    for x, y in aspiradoras:
        pygame.draw.rect(
            pantalla,
            COLOR_ASPIRADOR,
            (y * TAMANIO_CELDA, x * TAMANIO_CELDA, TAMANIO_CELDA, TAMANIO_CELDA),
        )

    # Mostrar estadísticas decoradas
